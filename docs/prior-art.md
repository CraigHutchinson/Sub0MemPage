...
| **PostgreSQL buffer-manager clock sweep** ([src/backend/storage/buffer/README](https://github.com/postgres/postgres/blob/master/src/backend/storage/buffer/README), fetched 2026-09-25) | The M2 initial replacement policy. *"To choose a victim buffer to recycle we use a simple clock-sweep algorithm"*; *"The \"clock hand\" is a buffer index, nextVictimBuffer, that moves circularly through all the available buffers"*; *"If the selected buffer is pinned or has a nonzero usage count, it cannot be used.  Decrement its usage count (if nonzero)... examine the next buffer."* The only stated rationale is contention: the pre-8.1 single BufMgrLock *"unsurprisingly proved to be a source of contention"*. Sub0MemPage uses the one-bit (second-chance) form of the same sweep: pinned and Filling slots are skipped, and the reference bit is set by resolve, not by prefetch (R6). No LRU list is reordered on access (R12). Wikipedia's Clock definition agrees: *"If R is 0, the new page is put in place of the page the \"hand\" points to... Otherwise, the R bit is cleared"*. | **High** (verbatim PostgreSQL source text) |
...
| **Linux MGLRU** | The single most load-bearing eviction-bookkeeping citation for this project's own API shape: the kernel does **not** ask anyone to report accesses — it reads hardware accessed bits in bulk, on its own schedule. Directly informs REQUIREMENTS.md R11 ("no per-access caller cooperation required"), because this project's real consumer is a hot loop that must call into the library zero times per access. | **Medium** |
