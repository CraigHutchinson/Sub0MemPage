# Reconciliation with Sub0TieredCache's API

Sub0TieredCache's [`README.md` §3](https://github.com/CraigHutchinson/Sub0TieredCache/blob/main/README.md) gives:
...
Sub0MemPage's scope because it stays Sub0TieredCache's job.

**The point of this exercise**: adopting Sub0MemPage underneath Sub0TieredCache should be a *substitution*, not a
...
- **`wait(ticket)` keeps its name and its "blocks the calling thread only, never a global lock" guarantee**
  (Sub0TieredCache README §3b).
- **The "only two calls may block on I/O" rule is preserved exactly**: the only calls that may block on I/O
  are `resolve` (Sub0MemPage's synchronous, pinning analog of Sub0TieredCache's `resolve_into`) and `wait`. Every
...
- **Coalescing is inherited unchanged** (Sub0TieredCache's R5): concurrent requests for overlapping ranges must
  produce one real I/O, not one per caller. At page granularity this is if anything more natural than at
  row granularity.
- **`stats` keeps its name and its "observability only, nothing depends on it" framing** (Sub0TieredCache's R10),
  extended with budget/headroom/high-water and the hint-efficacy counters Sub0TieredCache's own `stats` has no
  equivalent of today.

## D1 — Addressing unit: byte ranges, not `(table_id, row_index)`

**Necessary, and it is the layering line itself.** Sub0TieredCache's own R1 says it receives already-computed row
indices; Sub0MemPage sits one level below that and receives already-computed byte offsets. Sub0TieredCache's
...
descriptor is already the general form. So a Sub0TieredCache built on Sub0MemPage keeps its R7 offset-resolver
...
**No Sub0TieredCache requirement changes.**

## D2 — Explicit pinning via leases; Sub0TieredCache has no such concept today

**This is the substantive divergence, and the one Sub0TieredCache's maintainer actually has to think about.**
Sub0TieredCache can currently omit pinning because both of its read paths sidestep the lifetime question:
...
only because Sub0TieredCache's RAM tier is a private, never-evicted-mid-use structure. It becomes a real
...
semantics) is trivially implementable on top of `resolve` + `memcpy` + `release`, so Sub0TieredCache's own
...
Sub0TieredCache RAM tier built on Sub0MemPage would find `resolve_into` isn't bridging to a foreign concept at
all, it is passing its own contract straight through to the layer beneath it. If Sub0TieredCache ever wants
zero-copy `try_get` to be genuinely safe under a real budget, the lease is still the mechanism that makes
it so. **Adopting Sub0MemPage would close an existing latent hole in Sub0TieredCache's own spec, not open a new
...
Sub0TieredCache's README describes tiers a caller "configures per deployment" but names no budget parameter in
...
**Additive, and it now maps even more directly onto a decision a Sub0TieredCache deployment already has to make**
when sizing its RAM tier: Sub0TieredCache already has to allocate that tier's backing storage somewhere; under
this resolution, that same allocation *is* the Sub0MemPage pool's budget, registered once via
`register_slots`, rather than a size Sub0TieredCache allocates and a separate number it also tells Sub0MemPage to
enforce.

## D4 — `wont_need` and `on_evict` have no Sub0TieredCache counterpart

**Additive, lower layer only.** `wont_need` is meaningful because Sub0MemPage's caller can know a range is
finished with — a decoded layer's experts, once that layer is past — in a way Sub0TieredCache's row-level
abstraction has no natural place to express. `on_evict` exists mainly *for* a higher layer like Sub0TieredCache —
see design.md §4: if Sub0TieredCache's RAM tier were backed by a Sub0MemPage region, Sub0TieredCache would need to know a
range went away so it can drop its own row-level index entries pointing into it, without Sub0MemPage's
enforcement path ever depending on Sub0TieredCache's callback completing correctly (the veto-rejection reasoning
in design.md §4 applies here directly).

## D5 — Class-tagged prefetch (`DECLARED` vs `SPECULATIVE`)

Sub0TieredCache's `prefetch` has no class parameter today. **Additive and defaultable**: absent an explicit
class, treat a Sub0TieredCache-originated prefetch as `DECLARED`, which reproduces Sub0TieredCache's current semantics
exactly — Sub0TieredCache's own row-index-based prefetch calls are, definitionally, the caller having already
decided it wants those exact rows, the same shape as a `DECLARED` hint. Sub0TieredCache's own design doc
(`tiered-storage-design.md` §1a) already names both access shapes ("precomputed working-set" and
"reactive") as first-class, so the concept is already present in Sub0TieredCache's own design — Sub0MemPage just
gives it an explicit parameter one layer down.

## D6 — No `invalidate`, no `version_tag`, no dtype

**Deliberate omission — this stays entirely Sub0TieredCache's job.** Sub0TieredCache's R4 (value immutability between
...
interprets bytes at all — REQUIREMENTS.md R1 takes Sub0TieredCache's own R9 ("row content is opaque") one step
further: Sub0MemPage does not even know a row's width, let alone its dtype. A file-content change under a
live mapping is a caller error at Sub0MemPage's layer; detecting it is entirely Sub0TieredCache's `version_tag`,
...
The first backend accepts local files (an actual mapping is optional), so Sub0TieredCache's `remote_http_range(base_url, ...)` source
descriptor **cannot** be a Sub0MemPage region. A Sub0TieredCache built on Sub0MemPage would use it for the local-
...
**Sub0MemPage is not a complete substrate for Sub0TieredCache — it is a substrate for Sub0TieredCache's local tiers.**
Any adoption proposal that claims otherwise is overselling it.

## D8 — Optional `open_stream`/`next`

No Sub0TieredCache counterpart. Strictly additive, and the design's most speculative element (design.md §2, §5's
...
Adopting Sub0MemPage beneath Sub0TieredCache would require **no change to any of Sub0TieredCache's R1–R10**, would leave
...
all content semantics exactly where they already are (D6, D7). The only genuinely new concept Sub0TieredCache
would have to absorb internally, if it chose to adopt Sub0MemPage as its own local-tier substrate, is the
lease.

This is stated as a design analysis, not a commitment either project has made — Sub0TieredCache's own maintainer
is the one who decides whether or when to make this substitution, per Craig Hutchinson's own framing of
the relationship (quoted in [README.md](../README.md) §2): *"Sub0TieredCache could be another use-case for the
