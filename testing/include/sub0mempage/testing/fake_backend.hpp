 ...
 *
 *  Source bytes are a pure function of offset (source_byte), which is the independent oracle every
 *  lease is checked against. Bounded like a real backend: submit() fails once `capacity` requests are
 *  pending, and never allocates (storage is reserved at construction).
 */
